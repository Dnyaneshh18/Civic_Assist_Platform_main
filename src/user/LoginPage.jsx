...
export default function LoginPage() {
  ...
  const fillDemoOtp = (codeToFill) => {
    const target = String(codeToFill || '123456').slice(0, 6);
    const digits = target.split('');
    while (digits.length < 6) digits.push('0');
    setOtp(digits);
    setError('');
    setTimeout(() => handleVerifyOtp(digits.join('')), 100);
  };
        ...
        <div className="w-full max-w-[360px] relative z-10">
          ...
          {step === 'otp' && (
            <div>
              ...
              {/* Demo SMS Notification Banner with Auto-fill */}
                ...
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-blue-900 uppercase tracking-wider flex items-center gap-1.5">
                      <i className="fas fa-comment-sms text-blue-600" /> Demo SMS Notification
                    </span>
                  </div>
                  <span className="text-[11px] text-blue-600 font-semibold bg-blue-100 px-2 py-0.5 rounded-full">Active</span>
                </div>
                <p className="text-xs text-slate-600 mb-2">
                  Verification OTP code:
                </p>
                <div className="flex items-center justify-between gap-3 bg-white p-2.5 rounded-xl border border-blue-100 shadow-inner">
                  <span className="font-mono text-2xl font-black tracking-widest text-blue-700 select-all">
                    {demoOtp || '123456'}
                  </span>
                  <button
                    type="button"
                    onClick={() => fillDemoOtp(demoOtp || '123456')}
                    className="px-3.5 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 active:scale-95 rounded-lg shadow-sm transition-all flex items-center gap-1.5"
                  >
                    <i className="fas fa-bolt text-yellow-300 text-xs" /> Auto-fill
                  </button>
                </div>
                <p className="text-[11px] text-slate-400 mt-2 text-center">
                  💡 In demo mode, use the code above or type any 6 digits (e.g. 123456)
                </p>
              </div>

              <div className="flex gap-1.5 sm:gap-2.5 justify-between mb-5" onPaste={handleOtpPaste}>
                {otp.map((digit, i) => (
